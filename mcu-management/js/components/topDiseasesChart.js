/**
 * Top Diseases Chart Component
 * Displays top comorbid diseases as Bar Chart or List View
 *
 * Features:
...
import comorbidDiseaseService from '../services/comorbidDiseaseService.js';
import { showToast } from '../utils/uiHelpers.js';

class TopDiseasesChart {
  constructor() {
    this.chart = null;
    this.data = [];
    this.currentView = 'bar'; // 'bar' or 'list'
    this.limit = 10; // Default top 10
    this.containerId = 'top-diseases-container';
  }

  /**
   * Get category color based on disease type
   */
  getCategoryColor(category) {
    const colors = {
      'Comorbid Diseases': '#ef4444',     // Red
      'cardiovascular': '#dc2626',         // Dark Red
      'metabolic': '#f97316',              // Orange
      'respiratory': '#06b6d4',            // Cyan
      'infectious': '#8b5cf6',             // Purple
      'cancer': '#1f2937',                 // Dark Gray
      'other': '#6b7280'                   // Gray
    };
  ...
  getCategoryBgColor(category) {
    const colors = {
      'Comorbid Diseases': 'rgba(239, 68, 68, 0.1)',      // Red
      'cardiovascular': 'rgba(220, 38, 38, 0.1)',         // Dark Red
      'metabolic': 'rgba(249, 115, 34, 0.1)',             // Orange
      'respiratory': 'rgba(6, 182, 212, 0.1)',            // Cyan
      'infectious': 'rgba(139, 92, 246, 0.1)',            // Purple
      'cancer': 'rgba(31, 41, 55, 0.1)',                  // Dark Gray
  ...
  renderBarChart(container) {
    ...
    const canvasId = 'top-diseases-bar-chart';
          ...
          tooltip: {
            callbacks: {
              label: function(context) {
                return `${context.parsed.x} karyawan`;
              }
            }
          }
        },
          ...
          return `
            <div class="flex items-center gap-3 p-4 rounded-lg border-l-4" style="border-color: ${categoryColor}; background-color: ${categoryBgColor}">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-500">Penyakit Komorbid</p>
              </div>
  ...
  async render(filteredMCUs, options = {}) {
    try {
      ...
      // Get diseases data
      this.data = await comorbidDiseaseService.getTopComorbidDiseases(
        filteredMCUs,
        {
          limit: limit
        }
      ...
      const summaryStats = await comorbidDiseaseService.getComorbidDiseasesSummary(filteredMCUs);
      ...
      let html = `
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="flex items-center justify-between mb-6 pb-4 border-b">
            <div>
              <h3 class="text-lg font-semibold text-gray-900">Top ${limit} Penyakit Komorbid</h3>
              <p class="text-sm text-gray-500 mt-1">
                Total: ${summaryStats.totalDiseases} penyakit, ${summaryStats.totalOccurrences} karyawan
              </p>
            </div>
            ...
            <div class="flex items-center gap-4">
              <!-- Limit selector -->
              <select id="diseases-limit" class="px-3 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
              ...
              <div class="flex gap-2 bg-gray-100 p-1 rounded-lg">
                <button id="diseases-bar-btn" class="px-3 py-2 rounded font-medium transition-colors ${view === 'bar' ? 'bg-white text-blue-600 shadow' : 'text-gray-600 hover:text-gray-900'}">
                  📊 Chart
                </button>
                <button id="diseases-list-btn" class="px-3 py-2 rounded font-medium transition-colors ${view === 'list' ? 'bg-white text-blue-600 shadow' : 'text-gray-600 hover:text-gray-900'}">
                  📋 List
                </button>
              </div>
            </div>
          ...
          <div id="diseases-content" style="min-height: 300px;">
          ...
          <div class="mt-6 pt-4 border-t grid grid-cols-3 gap-4">
            <div class="bg-red-50 p-4 rounded-lg">
              <p class="text-xs text-red-600 font-medium">Total Penyakit</p>
              <p class="text-2xl font-bold text-red-900">${summaryStats.totalDiseases}</p>
            </div>
            <div class="bg-orange-50 p-4 rounded-lg">
              <p class="text-xs text-orange-600 font-medium">Total Karyawan</p>
              <p class="text-2xl font-bold text-orange-900">${summaryStats.totalOccurrences}</p>
            </div>
            <div class="bg-pink-50 p-4 rounded-lg">
              <p class="text-xs text-pink-600 font-medium">Paling Umum</p>
              <p class="text-sm font-bold text-pink-900 truncate" title="${summaryStats.mostCommon?.name || '-'}">
      ...
      const contentContainer = document.getElementById('diseases-content');
      if (view === 'bar') {
      ...
      if (container) {
        ...
        container.innerHTML = `<div class="text-red-600 p-4">
          <p><strong>Error loading comorbid diseases:</strong> ${errorMsg}</p>
  ...
  attachEventListeners() {
    // Limit selector
    const limitSelect = document.getElementById('diseases-limit');
    if (limitSelect) {
      limitSelect.addEventListener('change', (e) => {
        this.limit = parseInt(e.target.value);
        window.topDiseasesChartInstance?.updateView();
      });
    }

    // View toggle buttons
    const barBtn = document.getElementById('diseases-bar-btn');
    const listBtn = document.getElementById('diseases-list-btn');
...
export const topDiseasesChartInstance = new TopDiseasesChart();
